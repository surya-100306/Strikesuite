...
class BruteForcer:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    if result and result.get('success'):
                        ...
                        self.logger.warning(f"SSH credentials found: {result['username']}:{result['password']}")
            ...
            for future in as_completed(futures):
                try:
                    ...
                    if result and result.get('success'):
                        results['found_credentials'].append(result)
                        successful_attempts += 1
                        self.logger.warning(f"FTP credentials found: {result['username']}:{result['password']}")
            ...
            for future in as_completed(futures):
                try:
                    ...
                    if result and result.get('success'):
                        results['found_credentials'].append(result)
                        successful_attempts += 1
                        self.logger.warning(f"HTTP credentials found: {result['username']}:{result['password']}")
            ...
            for future in as_completed(futures):
                try:
                    ...
                    if result and result.get('success'):
                        results['found_credentials'].append(result)
                        successful_attempts += 1
                        self.logger.warning(f"{service} credentials found: {result['username']}:{result['password']}")
    ...
    def _test_mongodb_credentials(self, target: str, port: int, 
                                 username: str, password: str) -> Optional[Dict]:
        ...
        try:
            if MONGODB_AVAILABLE:
                ...
                client = pymongo.MongoClient(
                    f"mongodb://{username}:{password}@{target}:{port}/",
    ...
    def _test_redis_credentials(self, target: str, port: int, 
                              username: str, password: str) -> Optional[Dict]:
